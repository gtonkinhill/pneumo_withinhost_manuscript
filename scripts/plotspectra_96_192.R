...
plotspectrum192 = function(m, pdfname="spectrum.pdf", trinfreqs=NULL, plottype=1) {
    ...
    m$context_py <- m$context
    m$py = TRUE
    is_pyri <- m$ref %in% c('A', 'G')
    m$ref_py[is_pyri] <- ntcomp[m$ref[is_pyri]]
    m$mut_py[is_pyri] <- ntcomp[m$mut[is_pyri]]
    m$context_py[is_pyri] <- paste(ntcomp[substr(m$context[is_pyri],3,3)],ntcomp[substr(m$context[is_pyri],1,1)], sep="-")
    m$py[is_pyri] <- FALSE
